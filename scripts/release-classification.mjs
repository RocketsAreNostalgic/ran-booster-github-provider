...
export function assertReleaseClassification({ baseComposer, headComposer, releaseConfig, paths, title }) {
  ...
  if (!classification.breaking && !visible.has(classification.type)) {
    throw new Error(
      `release-significant provider changes require one of ${[...visible].join(", ")} or an explicit breaking ! classification; ${classification.type}: is hidden`,
    );
  }
...
export function runCli(root = process.cwd(), env = process.env) {
  ...
  if (result.required) {
    console.log(`release-significant provider change; ${result.classification.type}${result.classification.breaking ? "!" : ""}: is release-driving`);
  } else {
    console.log("no release-significant provider source or production requirement change");
  }
  return result;
}
