...
function firstNotes(changelog, version) {
  ...
  const heading = new RegExp(`^## (?:${escaped} ${date}|\\[${escaped}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-booster-github-provider/compare/v[^)\\s]+\\.\\.\\.v${escaped}\\) ${date})\\n`, "m");
...
export function candidateIdentity(contents, candidateSha) {
  ...
  if (composer?.name !== "ran/booster-github-provider" || composer?.type !== "library" || Object.hasOwn(composer, "version")) refuse("composer_identity_invalid", "Composer identity is not the unversioned target library");
